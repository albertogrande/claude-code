...
order: 0
summary: The one rule that changes everything, and the three habits that separate expert sessions from frustrating ones.
updated: 2026-07-04
---
...
1. **Frame precisely.** State scope, constraints, and success criteria up front. "Add a calendar widget following `HotDogWidget.php`, no new libraries" beats "add a calendar."
2. **Give a verification loop.** Tests, a build, a screenshot to diff. Without a pass/fail signal, *you* become the loop and Claude stops at "looks done."
3. **Protect the context.** The context window is the real budget. Clear between tasks, delegate exploration, and let memory do the remembering.
